...
class User {
	...
	protected static $db_fields = array('user_id', 'username', 'password', 'email', 'first_name', 'last_name', 'image1', 'latitude', 'longitude', 'image2', 'location');
	public $user_id;
	public $username;
	public $password;
	public $first_name;
	...
	public static function find_by_distance($latit, $longit, $dist, $myid) {

		return self::find_by_sql("SELECT *, 3956 * ACOS(COS(RADIANS($latit)) * COS(RADIANS(clients.latitude)) * COS(RADIANS(clients.longitude) - RADIANS($longit)) + SIN(RADIANS($latit)) * SIN(RADIANS(clients.latitude))) AS distance 
FROM clients 
HAVING distance < $dist AND user_id != $myid 
ORDER BY distance 
LIMIT 20");
		
	}
	
	public static function find_by_distance_pagination($page, $per_page, $total_count, $latit, $longit, $dist, $myid) {
